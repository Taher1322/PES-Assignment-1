 ...
 *    
 *
 *    File name   : uint_to_binstr.c
 *    Description : Returns binary represtation of an unsigned integer. Stores the binary representation in a char array
 *
...
#include <stdint.h>
 ...
 int uint_to_binstr(char *str, size_t size, uint32_t num, uint8_t nbits)   //Function to return binary value of an unsigned integer
{
    int bin_value;                                                         //Defining some local variables for calculations
    int max_value = 1;
    int base = 2;
    if(nbits !=0 && num >=0)                                               //Checking boundary conditions
    {
        for (int i = nbits-1; i>=0; i--)                                   //For loop to calculate 2^n value 
        {
            max_value = max_value * base;
        }
        if (max_value > num)                                               //Checking if the nbit is in the range of the number 
        {   
